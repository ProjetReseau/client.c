...
void * connexion(void* envoy){	//Thread de connexion, 1 par connexion client-client
  ...
  while(1){
	...
	break;
		...
		sprintf(datas, "Proposition d'envoi de %s par %s", name_file_r, envoi->pseudo);
		affichier_haut(datas);
	}
    ...
    if(!(estVide_fifo(envoi))){
      ...
      recup_nom(trame_write.message,name_file_r);

      if (strcmp(trame_write.message,name_file_r)!=0){
